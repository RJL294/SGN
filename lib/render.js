...
// Inside a .photo box: the article's real image if we have one (with the
// gradient showing through if it fails to load), otherwise the glyph.
function photoInner(a) {
  if (a.image) {
    return `<img class="photo-img" src="${esc(a.image)}" alt="" loading="lazy" referrerpolicy="no-referrer" onerror="this.remove()">`;
  }
  return `<span class="glyph" aria-hidden="true">${a.glyph}</span>`;
}
...
function utilBar() {
  ...
  <div class="util">
    <div class="wrap">
      <span class="date" id="today">${new Date().toLocaleDateString('en-US',{weekday:'long',month:'long',day:'numeric',year:'numeric'}).toUpperCase()}</span>
  ...
  <header class="masthead">
    <div class="wrap">
      <div class="nameplate">
        ...
        <h1 class="wordmark"><a href="${homeHref(base)}">Cloudbreak <span class="good">News</span></a></h1>
      </div>
      <p class="dateline">
      ...
      <nav>
        <div class="navrow">
          ...
          <a href="${base}news.html"${activeSlug === 'news' ? ' class="active"' : ''}>All News</a>
${links}
        </div>
      </nav>
    </div>
...
function footer(base = '') {
  ...
  <footer>
    <div class="wrap">
      ...
      <div class="legal">
        <span class="wm">Cloudbreak <em>News</em></span>
        <span class="spacer"></span>
...
function card(a, lg, base) {
  ...
  return `        <article class="card${lg ? ' lg' : ''}">
          <a class="photo" href="${storyHref(base, a.id)}" style="background:${a.gradient};">${photoInner(a)}</a>
...
function block(sec, cards, base) {
  const moreHref = sec.slug === 'latest'
    ? `${base}news.html`
    : NAV_SLUGS.has(sec.slug)
    ? sectionHref(base, sec.slug)
    : '#';
  return `
  <section class="block">
    <div class="wrap">
...
// A build-time inline SVG line chart — axis, a dot and value label per data
// point, and year labels at each end. No chart library; scales with the card.
function lineChart(series) {
  const W = 320, H = 164, padX = 16, padTop = 24, padBottom = 30;
  const years = series.map((p) => p.year);
  const vals = series.map((p) => p.value);
  const minY = Math.min(...years), maxY = Math.max(...years);
  const minV = Math.min(...vals), maxV = Math.max(...vals);
  const plotBottom = H - padBottom;
  const sx = (y) => padX + (maxY === minY ? 0 : (y - minY) / (maxY - minY)) * (W - 2 * padX);
  const sy = (v) => plotBottom - (maxV === minV ? 0 : (v - minV) / (maxV - minV)) * (plotBottom - padTop);
  const pts = series.map((p) => `${sx(p.year).toFixed(1)},${sy(p.value).toFixed(1)}`);
  const area = `${sx(minY).toFixed(1)},${plotBottom} ${pts.join(' ')} ${sx(maxY).toFixed(1)},${plotBottom}`;
  const dots = series
    .map((p) => `<circle class="chart-dot" cx="${sx(p.year).toFixed(1)}" cy="${sy(p.value).toFixed(1)}" r="3"/>`)
    .join('');
  // value label above each point; nudge the end points inward so they don't clip
  const vlabels = series
    .map((p, i) => {
      const anchor = i === 0 ? 'start' : i === series.length - 1 ? 'end' : 'middle';
      return `<text class="chart-vlabel" x="${sx(p.year).toFixed(1)}" y="${(sy(p.value) - 7).toFixed(1)}" text-anchor="${anchor}">${p.value}</text>`;
    })
    .join('');
  // year label under every data point
  const xlabels = series
    .map((p, i) => {
      const anchor = i === 0 ? 'start' : i === series.length - 1 ? 'end' : 'middle';
      return `<text class="chart-xlabel" x="${sx(p.year).toFixed(1)}" y="${H - 8}" text-anchor="${anchor}">${p.year}</text>`;
    })
    .join('');
  const last = series[series.length - 1];
  return `<svg class="chart" viewBox="0 0 ${W} ${H}" role="img" aria-hidden="true">
        <line class="chart-axis" x1="${padX}" y1="${plotBottom}" x2="${W - padX}" y2="${plotBottom}"/>
        <polyline class="chart-area" points="${area}"/>
        <polyline class="chart-line" points="${pts.join(' ')}"/>
        ${dots}
        <circle class="chart-dot chart-dot-last" cx="${sx(last.year).toFixed(1)}" cy="${sy(last.value).toFixed(1)}" r="4.5"/>
        ${vlabels}
        ${xlabels}
      </svg>`;
}
...
function metricCard(m) {
  ...
  return `      <article class="metric">
      ...
      <div class="metric-figure">
        ...
        <span class="metric-then">from ${esc(firstDisp)} in ${first.year}${m.unitNote ? ' · ' + esc(m.unitNote) : ''}</span>
      </div>
      ${lineChart(m.series)}
...
export function renderProgress({ progress }) {
  ...
  return doc({ title: 'The Big Picture — Cloudbreak News', body, base });
}
...
export function renderHome({ home, byId, progress }) {
  ...
  return doc({ title: 'Cloudbreak News — Good news only', body, base });
}
...
${cards}
  ...
  return doc({ title: `${label} — Cloudbreak News`, body, base });
}
...
function dayLabel(key, todayKey, yestKey) {
  ...
  return `${wd}, ${MONTHS[m - 1]} ${d}${y === thisYear ? '' : ', ' + y}`;
}

export function renderAllNews({ articles }) {
  const base = '';
  const count = articles.length;
  const now = Date.now();
  const todayKey = dayKey(now);
  const yestKey = dayKey(now - 86400000);

  // group articles (already newest-first) by the day they're dated
  const groups = [];
  let current = null;
  for (const a of articles) {
    const key = dayKey(now - a.hoursAgo * 3600000);
  ...
  const groupsHtml = groups
    .map(
      (g) => `      <section class="news-day">
        <h2 class="news-date">${dayLabel(g.key, todayKey, yestKey)}</h2>
        <div class="grid3">
${g.items.map((a) => card(a, false, base)).join('\n')}
        </div>
      </section>`
    )
    .join('\n');
  ...
  const main = count
    ? `${groupsHtml}\n      <div class="news-more"><button id="load-more" class="btn-lg" type="button" hidden>Load older stories →</button></div>`
    : `      <p class="empty">No stories yet — the feed fills in as the daily refresh runs. ☀</p>`;
  ...
  <div class="wrap">
    <div class="section-hero">
      <span class="kicker">${count} ${count === 1 ? 'story' : 'stories'}</span>
      <h1>All News</h1>
      <p class="section-sub">Every story we're following, newest first. Fresh ones land at the top; older ones scroll down until they retire after three months.</p>
    </div>
  </div>`;
  ...
  return doc({ title: 'All News — Cloudbreak News', body, base });
}
...
export function renderArticle({ article: a, related }) {
  ...
  <div class="wrap">
    <article class="story">
      ...
      <p class="byline">${meta}</p>
      <div class="photo story-photo" style="background:${a.gradient};">${photoInner(a)}</div>
...
${bodyParas}
  ...
  return doc({ title: `${a.title} — Cloudbreak News`, body, base });
}
...
export function renderAbout() {
  ...
  return doc({ title: 'About — Cloudbreak News', body, base });
}
...
export function renderTip({ endpoint = '', contactEmail = '' } = {}) {
  ...
  const body = [utilBar(), masthead(null, base), page, footer(base)].join('\n');
  return doc({ title: 'Send us a tip — Cloudbreak News', body, base });
}
