  ...
  <header class="masthead">
    <div class="wrap">
      <div class="nameplate">
        ...
        <h1 class="wordmark"><a href="${homeHref(base)}">Some <span class="good">Good</span> News</a></h1>
      </div>
      <p class="dateline">
...
export function renderHome({ home, byId }) {
  ...
  return doc({ title: 'Some Good News — Good news only', body, base });
}
...
${cards}
  ...
  return doc({ title: `${label} — Some Good News`, body, base });
}
...
export function renderArticle({ article: a, related }) {
  ...
  const sourceCta =
    a.url && a.url !== '#'
      ...
      : `\n      <p class="source-note">Curated by Some Good News — we link straight to the original source as soon as it's available.</p>`;
...
${bodyParas}
  ...
  return doc({ title: `${a.title} — Some Good News`, body, base });
}
